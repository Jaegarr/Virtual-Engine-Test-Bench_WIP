...
def emission_plots(df):
    ...
    fig.set_ylabel('Emissions (g/kWh)')
    fig.plot(df['Engine Speed (RPM)'], df['CO2(g/s)']*3600/df['Power (kW)'], color='magenta', label='CO2')
    fig.plot(df['Engine Speed (RPM)'], df['CO(g/s)']*3600/df['Power (kW)'], color='tab:red', label='CO')
    fig.plot(df['Engine Speed (RPM)'], df['NOx(g/s)']*3600/df['Power (kW)'], color='tab:blue', label='NOx')
    fig.plot(df['Engine Speed (RPM)'], df['HC(g/s)']*3600/df['Power (kW)'], color='tab:green', label='HC')
    fig.legend()
    plt.title('Emissions')
    plt.show()