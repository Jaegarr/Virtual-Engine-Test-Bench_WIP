...
def calculate_torque(rpm, throttle, mdotAir, displacement_l, LHV = 44e6, eff = 0.3):
    ...
    torque_fmep = cal.calculate_fmep(rpm, displacement_l)
    torque_pmep = cal.calculate_pmep(rpm, displacement_l)
    net_torque = gross_torque - torque_fmep - torque_pmep
    return max(net_torque,0) # I am not confident with myself sometimes
